...
- **`llm_reasoning.py`**: despite the name, `generate_evaluation_heuristic` is the only implementation — there is no real LLM call. It produces a 0-10 score from literature mention count, Lipinski compliance, MW range, and LogP range. The `DRUG_EVALUATION_SCHEMA` and prompt templates exist but are unused scaffolding for a future LLM backend (DSPy/Outlines/llama.cpp are commented out in `requirements.txt`).
